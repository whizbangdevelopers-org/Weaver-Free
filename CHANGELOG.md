...
- **`audit:published-entrypoints` now follows the runtime dispatcher it previously stopped at.** `run-compliance.ts` (which `test:compliance` invokes) fans out to ~65 auditors via `spawnSync('npm', ['run', name])` over its `PHASES` list; it is published, but nearly every auditor it dispatches is sync-excluded. The static resolver stopped at the published dispatcher, so an ungated published `npm run test:compliance` would spawn a missing auditor file on the mirror — the same false-alarm class as #340 — while the audit reported PASS. It was latent (both `test:compliance` references are Dev-gated) but one ungated step from live. The resolver now walks `run-compliance`'s fan-out via its exported `PHASES` (a stable data import, not a re-parse), proven by a synthetic ungated reach surfacing 41 unpublished scripts through `run-compliance⋯ → audit:* → <file>`. `run-compliance` is the sole runtime npm-run fan-out in the tree; a new one must be added to `DISPATCHERS`.
- **A generated artifact was non-deterministic in a shallow clone, and failed silently.** `generate-feature-lifecycle-views.ts` dates its `**Updated:**` line from the schema's last commit (`git log -1 --format=%cs`) specifically to avoid a per-run timestamp. In a `--depth 1` checkout that is not deterministic: the shallow boundary commit looks like the commit that introduced every path, so git reports the *tip* commit's date — no error and no empty string, so the `|| new Date()` fallback never fired. The generator produced a different, entirely plausible date and both artifacts read as stale. `actions/checkout` defaults to depth 1, so this failed only inside the tag-triggered release pipeline (unrun since 2026-04-26) while passing on every developer machine. The generator now refuses a shallow clone with a message naming the fix instead of guessing, and `release.yml`'s `verify` and `build` jobs check out with `fetch-depth: 0`.
