    ...
    <q-tab-panels v-model="activeTab" animated keep-alive>
      ...
      <q-tab-panel name="custom" class="q-pa-none">
        <div class="row q-col-gutter-md">
          ...
          <!-- Custom MicroVM — v2.0+ enables Create -->
          <div class="col-12 col-sm-6 col-md-4">
            <q-card flat bordered class="shed-card">
              <q-card-section class="row items-center no-wrap q-pb-xs">
              ...
              <q-card-actions class="q-pa-sm">
                <q-btn
                  ...
                  :disable="!atLeast('2.0')"
                >
                  <q-tooltip v-if="!atLeast('2.0')">Custom creation available in v2.0</q-tooltip>
                </q-btn>
              </q-card-actions>
            </q-card>
          </div>
            ...
            <div class="row q-gutter-md">
              <q-card flat bordered class="col" style="min-width:200px">
                <q-card-section class="text-center q-pa-md">
                  ...
                  <div class="text-caption text-grey">Proxmox · Docker Compose · libvirt XML · Dockerfile</div>
              ...
              <q-card flat bordered class="col" style="min-width:200px">
                <q-card-section class="text-center q-pa-md">
                  ...
                  <div class="text-caption text-grey">NixOS config · JSON manifest · tar.gz bundle</div>
...
const templates: ShedTemplate[] = [
  // Wave 1
  { id: 'nextcloud', name: 'Nextcloud', description: 'File sync, calendar, contacts, and collaboration. Drop-in Google Workspace replacement.', detail: 'Composed template: nginx + PostgreSQL + Valkey. One click deploys the full stack.', icon: 'mdi-cloud-outline', color: 'blue-7', tier: TIERS.WEAVER, wave: 1 },
  { id: 'forgejo', name: 'Forgejo', description: 'Self-hosted Git. Full GitHub-compatible API, CI runners, issue tracker, and web UI.', detail: '"Deploy GitHub in 30 seconds." Lightweight, MIT-licensed Git forge.', icon: 'mdi-source-branch', color: 'orange-8', tier: TIERS.WEAVER, wave: 1 },
  { id: 'authentik', name: 'Authentik', description: 'Identity hub for your homelab. One Authentik VM gives every app SSO.', detail: 'Run one Authentik VM. Every application gets SSO.', icon: 'mdi-shield-account', color: 'indigo-6', tier: TIERS.WEAVER, wave: 1 },
  { id: 'pihole', name: 'Pi-hole', description: 'Network-wide ad, tracker, and malware blocking for every device on your LAN.', detail: 'Pi-hole = all-device network DNS; dns extensions = VM fleet service discovery.', icon: 'mdi-shield-outline', color: 'red-6', tier: TIERS.WEAVER, wave: 1 },
  { id: 'media-stack', name: 'Media Stack', description: 'Jellyfin + Navidrome + Immich + Audiobookshelf. Complete self-hosted media library.', detail: 'Composed template. Single-click replacement for Plex, Google Photos, and Spotify.', icon: 'mdi-play-box-multiple', color: 'deep-purple-6', tier: TIERS.WEAVER, wave: 1 },
  { id: 'home-assistant', name: 'Home Assistant', description: 'Open-source home automation hub. Integrates with 3,000+ devices and services.', detail: 'Isolated from the host — HA crashes never touch your other workloads.', icon: 'mdi-home-automation', color: 'teal-6', tier: TIERS.WEAVER, wave: 1 },
  { id: 'postgresql', name: 'PostgreSQL', description: 'Production-grade relational database. Dependency of Nextcloud and other composed templates.', detail: 'Standalone template. Weaver manages the VM lifecycle; you manage the schema.', icon: 'mdi-database', color: 'blue-grey-7', tier: TIERS.WEAVER, wave: 1 },
  { id: 'nginx', name: 'nginx', description: 'Reverse proxy and load balancer. Route traffic to any workload on your host.', detail: 'Replaces the K8s ingress controller use case for single-host deployments.', icon: 'mdi-web', color: 'green-7', tier: TIERS.WEAVER, wave: 1 },
  ...
  // Wave 2 (v2.1)
  { id: 'prometheus', name: 'Prometheus + Grafana', description: 'Metrics collection and visualization stack. Monitor all your workloads.', detail: 'Composed template: Prometheus + Grafana + node_exporter.', icon: 'mdi-chart-line', color: 'orange-6', tier: TIERS.WEAVER, wave: 2 },
  { id: 'vaultwarden', name: 'Vaultwarden', description: 'Self-hosted Bitwarden-compatible password manager. Zero cloud dependency.', detail: 'Lightweight Rust implementation. Full Bitwarden client compatibility.', icon: 'mdi-lock', color: 'blue-8', tier: TIERS.WEAVER, wave: 2 },
  { id: 'node-red', name: 'Node-RED', description: 'Flow-based automation and IoT integration. Wire together APIs, services, and devices.', detail: 'Low-code event processing. Pairs well with Home Assistant and MQTT.', icon: 'mdi-resistor-nodes', color: 'red-8', tier: TIERS.WEAVER, wave: 2 },
  { id: 'haproxy', name: 'HAProxy', description: 'High-availability load balancer with health checking and SSL termination.', detail: 'Production-grade load balancing for high-availability workloads.', icon: 'mdi-scale-balance', color: 'green-8', tier: TIERS.WEAVER, wave: 2 },
]
