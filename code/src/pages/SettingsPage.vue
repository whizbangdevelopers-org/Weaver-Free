      ...
      <q-card v-if="authStore.isAdmin" flat bordered>
        <q-expansion-item icon="mdi-stethoscope" label="System Health" caption="Hardware readiness diagnostics" header-class="text-h6">
          <q-card-section>
            ...
            <div v-else-if="!doctorLoading" class="text-caption text-grey">
            ...
            <div class="q-gutter-md">
              ...
              <q-markup-table v-if="licenseActivated" flat dense bordered separator="horizontal">
                <tbody>
                  ...
                  <tr v-if="appStore.effectiveTier === TIERS.WEAVER && appStore.demoWeaverSubTier === 'team'"><td class="text-weight-medium">Seats</td><td>4 users + 1 viewer</td></tr>
                  ...
                  <tr v-if="mockFmSlot"><td class="text-weight-medium">Program</td><td><q-badge color="amber-9" label="Founding Member" /> <span class="text-caption text-grey q-ml-xs">#{{ mockFmSlot.current }} of {{ mockFmSlot.cap }}</span></td></tr>
                  <tr><td class="text-weight-medium">Pricing</td><td>{{ mockLicensePricing }} <span v-if="mockFmSlot" class="text-caption text-grey q-ml-xs">(standard: {{ mockStandardPricing }}) — locked forever</span></td></tr>
        ...
        <q-expansion-item icon="mdi-robot" label="AI Default" caption="Server-side AI key status" header-class="text-h6">
          <q-card-section>
            <div class="q-gutter-md">
              ...
              <div v-if="!appStore.hasServerKey" class="text-caption text-grey">
      ...
      <q-card v-if="isTeamOrFabrick && isDemoMode() && appStore.isDemoVersionAtLeast('1.4')" flat bordered>
        <q-expansion-item icon="mdi-shield-key" label="AI Credential Vault" caption="Admin-managed AI credentials for the team" header-class="text-h6">
          <q-card-section>
            ...
            <div v-if="appStore.isFabrick" class="text-caption text-grey q-mt-sm">
        ...
        <q-expansion-item icon="mdi-linux" label="Distributions & Image URLs" caption="Cloud images for VM provisioning" header-class="text-h6">
          <q-card-section>
            <div v-if="appStore.isWeaver" class="row q-gutter-sm q-mb-md">
              ...
              <div v-if="urlStatus.lastRunAt" class="self-center text-caption text-grey">URLs last checked: {{ formatRelativeTime(urlStatus.lastRunAt) }}</div>
            </div>
            ...
            <div v-else class="text-grey">{{ appStore.isWeaver ? 'No distributions loaded.' : 'No distributions detected on registered VMs.' }}</div>
          ...
          <q-card-section>
            ...
            <template v-else>
              <div class="row items-center q-gutter-md">
                <q-icon name="mdi-lock" size="32px" color="grey-5" />
                <div class="text-body2 text-grey">Per-user resource quotas (VMs, memory, vCPUs) are available on the Fabrick tier.</div>
              </div>
            </template>
          </q-card-section>
        </q-expansion-item>
...
const MOCK_LICENSE_KEYS: Record<string, string> = {
  [TIERS.WEAVER]:  'WVR-WVS-3N7R2-F4J8L',
...
const mockLicenseKey = computed(() => {
  if (appStore.effectiveTier === TIERS.WEAVER && appStore.demoWeaverSubTier === 'team') return MOCK_LICENSE_KEYS_TEAM
