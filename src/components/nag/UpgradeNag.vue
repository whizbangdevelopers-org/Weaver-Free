...
const defaultDescription = computed(() => {
  const label = TIER_LABELS[props.requiredTier] ?? 'Weaver'
  if (publicDemo) {
    return props.requiredTier === TIERS.FABRICK
      ? `${props.featureName} is coming in the ${label} edition.`
      : `${props.featureName} is coming soon in the ${label} edition.`
  }
  return `This feature requires a ${label} license.`
})
