  ...
  <q-card flat bordered>
    ...
    <q-card-section>
      ...
      >
        ...
        <div class="text-body2 text-weight-medium">{{ hostConfig.error ?? 'NixOS configuration file unavailable' }}</div>
        <div class="text-caption q-mt-xs text-grey-8">Path: {{ hostConfig.configPath }}</div>
        <div v-if="hostConfig.resolvedPath && hostConfig.resolvedPath !== hostConfig.configPath" class="text-caption text-grey-8">
          Resolves to: {{ hostConfig.resolvedPath }}
        </div>
        <div v-if="hostConfig.remediation" class="q-mt-sm text-body2">
          <div class="text-weight-medium q-mb-xs">How to fix:</div>
          <div class="remediation-text">{{ hostConfig.remediation }}</div>
        </div>
      </q-banner>
          ...
          <div style="width: 220px; flex-shrink: 0">
            <q-list dense separator bordered rounded>
              ...
              <q-item v-if="visibleSections.length === 0">
                <q-item-section>
                  <q-item-label caption class="text-grey-7">No workload sections detected</q-item-label>
                </q-item-section>
              </q-item>
            </q-list>
          </div>
...
/* Remediation text preserves line breaks and renders embedded commands as monospace */
.remediation-text {
