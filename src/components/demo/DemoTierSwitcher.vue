...
<template>
  <div class="demo-tier-switcher row items-center no-wrap">
    <!-- Tier buttons hidden in public demo — the funnel lives in the version switcher -->
      ...
      >
        ...
        <q-tooltip anchor="bottom middle" self="top middle" :offset="[0, 8]">
          ...
          <template v-else>
            ...
            <div class="text-caption text-grey-7 q-mb-xs">{{ opt.persona }}</div>
...
// Public demo funnel routes — tier buttons navigate here instead of switching tiers
...
const SOLO_OPT: TierOption = {
  ...
  tier: TIERS.SOLO as Tier, subTier: 'solo',
...
const TEAM_OPT: TierOption = {
  ...
  tier: TIERS.SOLO as Tier, subTier: 'team',
  stage: DEMO_TIER_STAGES.weaver,
...
function isTierActive(opt: TierOption): boolean {
  // Public demo: active state based on current route
  if (isPublic.value) {
...
function isTierDisabled(tier: Tier): boolean {
  ...
  if (tier === TIERS.SOLO && appStore.isDemoVersionAtLeast('3.0')) return true
  return false
}
...
function selectTier(opt: TierOption) {
  // Public demo: navigate to funnel pages instead of switching tiers
  if (isPublic.value) {
    const route = PUBLIC_FUNNEL_ROUTES[opt.key]
