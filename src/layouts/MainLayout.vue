...
<template>
  ...
  <q-layout view="lHh Lpr lFf" :class="{ 'demo-layout': isDemoMode }">
    <q-header elevated :class="headerBgClass">
      <!-- DemoBanner removed from public demo — info is in the toolbar label (internal decision) -->
      ...
      <q-toolbar data-toolbar="fabrick">
        ...
        <!-- DemoTierRoadmap removed — funnel now in version switcher (internal decision) -->
    ...
    <q-drawer v-model="leftDrawerOpen" bordered>
      <q-list>
        ...
        <!-- Shed — unified workload creation (internal decision). Replaces New Workload button. Role-gated: hidden for Viewer/Auditor. -->
      ...
      <!-- Drawer footer -->
      <div class="absolute-bottom q-pa-md text-caption text-grey-8">
...
// NOTE: `pageTitle` references `appName` (defined ~90 lines below). The
// closure captures the identifier lazily so declaration order is fine for
// the computed itself — BUT `useMeta` eagerly evaluates the passed getter
// once to set the initial title, which triggers pageTitle → appName.value
// before `appName` is initialized (TDZ). The `useMeta` call is deferred
// until after `appName` is declared; search for "DEFERRED useMeta".
const pageTitle = computed(() => {
  const meta = route.meta as Record<string, unknown>
  if (meta.title) return meta.title as string
  // Workload detail page: show workload name with tier-qualified context
  if (route.path.startsWith('/workload/') && route.params.name) {
    return `${appName.value} — ${route.params.name as string}`
  }
...
const appName = computed(() => {
  ...
  // internal decision: toolbar shows tier qualifier — Free, Solo, Team
  ...
  // Real install: toolbar always shows the tier label.
  // If branded, prefix with the org name: "Acme Weaver Solo" / "Acme Weaver Free"
  const label = TIER_LABELS[appStore.effectiveTier as TierName] ?? 'Weaver'
  if (appStore.organization?.name) return `${appStore.organization.name} ${label}`
  return label
})
// DEFERRED useMeta: must come after `appName` declaration — see TDZ note above.
