...
export function getDemoNotificationsForTier(tier: string): NotificationEvent[] {
  switch (tier) {
    ...
    case TIERS.SOLO: return PREMIUM_NOTIFICATIONS
...
export function getDemoContainersForTier(tier: string): ContainerInfo[] {
  switch (tier) {
    ...
    case TIERS.SOLO: return PREMIUM_CONTAINERS
...
export const DEMO_VERSIONS: DemoVersionInfo[] = [
  ...
  { version: '2.0', label: 'v2.0.0', status: 'planned',     targetDate: DELIVERY_TARGET_DATES['2.0.0']!, headline: 'Storage + Templates', tierCeiling: TIERS.SOLO },
...
export function getDemoMetricsForVm(vmName: string, tier: string): VmMetrics {
  const is24h = tier === TIERS.SOLO || tier === TIERS.FABRICK
