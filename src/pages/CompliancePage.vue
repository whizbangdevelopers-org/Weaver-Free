...
<template>
  <q-page padding>
    ...
    <div class="q-gutter-md" style="max-width: 800px">
      ...
      <!-- Disclaimer -->
      <div class="text-caption text-grey-8 q-mt-lg q-pt-md" style="max-width: 800px">
...
const standards: ComplianceStandard[] = [
  {
    ...
    color: 'deep-purple',
    description: '25 controls across 7 families (Access Control, Audit, Identification & Authentication, System & Communications Protection, System Integrity, Configuration Management, Media Protection).',
    implemented: 20, total: 25, fullVersion: '2.3',
    planned: 'MFA, cryptographic key management, CUI encryption at rest, media protection, Approved Software List (3.4.8, v2.3 via Private Nix Cache)',
  },
  {
    slug: 'hipaa-164-312',
    title: 'HIPAA §164.312',
    ...
    implemented: 11, total: 14, fullVersion: '2.3',
    planned: 'MFA, encryption at rest, software integrity verification for ePHI-processing systems (§164.312(c)(1), v2.3 via Private Nix Cache)',
  },
  {
    slug: 'pci-dss',
    title: 'PCI DSS v4.0',
    ...
    color: 'blue',
    description: '28 controls across 6 requirements — secure configuration, data protection, secure development, access restriction, authentication, logging.',
    implemented: 24, total: 28, fullVersion: '2.3',
    planned: 'MFA for CDE access, cryptographic key protection, approved software change control (Req 6.4.5, v2.3), file-integrity monitoring (Req 11.5.1, v2.3 via Private Nix Cache)',
  },
  {
    slug: 'cis-benchmarks',
  ...
  },
  {
    ...
    color: 'cyan-8',
    description: 'Top-18 critical security controls — asset inventory, software allowlisting, secure configuration, access control, audit log management, data recovery, network infrastructure, application software security. (5 controls N/A — people/process/endpoint.)',
    implemented: 7, total: 13, fullVersion: '2.7',
    planned: 'Software allowlisting (Control 2, v2.3 via Private Nix Cache), data recovery (Control 11, v2.5 Backup Weaver compliance-complete), fleet audit aggregation (Control 8 fleet, v2.4), automated test restores (Control 11.5, v2.7 Backup FabricK)',
  },
  {
    slug: 'soc2-readiness',
    ...
    implemented: 25, total: 29, fullVersion: '2.3',
    planned: 'MFA, cryptographic key management, encryption at rest, unauthorized software prevention (CC6.8, v2.3 via Private Nix Cache)',
  },
]
...
function isFullCompliance(standard: ComplianceStandard): boolean {
  if (isDemoMode()) return appStore.isDemoVersionAtLeast(standard.fullVersion)
  // Proper semver comparison: parse major.minor and compare as integers
  const parse = (v: string): number => {
    const [maj, min] = v.replace(/^v/, '').split('.').map(Number)
    return (maj ?? 0) * 1000 + (min ?? 0)
  }
  return parse(__APP_VERSION__) >= parse(standard.fullVersion)
}
</script>
