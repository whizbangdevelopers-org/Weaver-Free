...
>
> **Adoption is one-way, and it interacts with the cap.** A scan ADDs and refreshes registry rows; nothing in the scan path removes one, and `DELETE /api/workload/:name` requires Solo. So undeclaring a workload in your NixOS config — or renaming it — does **not** free its slot: the original row stays registered and keeps counting toward the alphabetical-first 10. On Free the count can therefore only grow. Plan the workload names before the tenth, or budget for Solo.
