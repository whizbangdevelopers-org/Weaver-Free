...
**Fix:** Shared `testing/e2e-docker/scripts/prune-docker-artifacts.sh` runs `docker image prune -f` (dangling only) + `docker builder prune -f --filter until=168h`, wired into **all nine building runners** — not just the obvious ones: the six batch runners (after their final `down`), `run-single`/`run-ui` (via an `EXIT` trap so cleanup runs even when a test fails or the session is Ctrl-C'd), and `run-iterate` (dangling-only prune after each (re)build — its container stays alive by design, so a `down` would defeat it). **Deliberately NOT `docker image prune -a`** in the recurring path — `-a` deletes the freshly built *tagged* image too (no container references it once the run ends), forcing a full no-cache rebuild next run. Use `-af` only for a one-time reclaim; steady-state teardown uses `-f` so the current image stays for cache reuse and only the superseded `<none>` layer is dropped. Bounds the set to one image per service instead of one per run.

The trap caught a real gap: the first pass wired only the six runners with an obvious `down`. A second host (foundry) still showed dangling `e2e-docker-playwright-single` images because `run-single.sh` has no `down` teardown at all. **Cover every script that runs `--build`, not just the ones that already tear down.**
