...
# Node modules path: env override (set by NixOS module wrapper) or repo source dir
BACKEND_NODE_MODULES="${WEAVER_NODE_MODULES:-$REPO_ROOT/backend/node_modules}"
...
# Detect the configured service user/group from systemd so ownership matches
# whatever the NixOS module was set to (default 'weaver', or override).
SERVICE_USER=$(systemctl show weaver -p User --value 2>/dev/null || echo "")
SERVICE_GROUP=$(systemctl show weaver -p Group --value 2>/dev/null || echo "")
SERVICE_USER="${SERVICE_USER:-weaver}"
SERVICE_GROUP="${SERVICE_GROUP:-weaver}"
